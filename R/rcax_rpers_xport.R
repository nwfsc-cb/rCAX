#' Return RperS table from CAP Fish HLIs Tabular Query
#'  
#' @export
#' @template info
#' @template tableargs
...
#' id <- rcax_rpers_xport(qlist=list(limit=1))$nmfs_popid
#' 
#' a <- rcax_rpers_xport(
#'          flist=list(nmfs_popid=id),
#'          cols=c("nmfs_popid", "broodyear", "rpers")
#'          )
#' head(a)
#' 
#' # First 3 columns
#' rcax_rpers_xport(qlist=list(limit=3))[,1:3]
#' 
#' # to print the first column names and definitions
#' head(rcax_rpers_xport(type="colnames"))
#' 
#' # to print the first 5 column names as they appear
#' # in Excel files downloaded from https://www.streamnet.org/data/hli/
#' tab <- rcax_rpers_xport(qlist=list(limit=1))
#' colnames(tab)[1:5]
#' 
rcax_rpers_xport <- function(
    tablename = "XPortCA_RperS_01",
    flist = NULL,
    qlist = NULL, 
    cols = NULL,
  ...
  if(is.null(cols)) cols <- c(rpers_xport_colnames, "species", "publish", "agency", "hli_id")
