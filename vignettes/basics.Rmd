...
See the [Getting Started vignette](articles/setup.html) for instructions on installing and setting-up your API key. Once you have installed {rCAX} and set up your key, you can begin using the package by loading the library.
...
* `rcax_hli_xport()` The HLI tables with extra meta data like NMFS_PopID as returned from the [CAP Fish HLIs Tabular Query](https://www.streamnet.org/data/hli/)
* `rcax_hli()` The base HLI tables
...
Show the column names for the XPort versus base tables. The XPort tables have extra meta data.
```{r}
basecols <- rcax_hli("NOSA", type="colnames")$name
xportcols <- rcax_hli_xport("NOSA", type="colnames")$name
xportcols[which(!(xportcols %in% basecols))]
```

## Get records from the NOSA XPort table

Here the columns returned are restricted by `cols`. The table is filtered with `flist` to be just the columns with `nmfs_popid` equal to 7. Note the `cols` argument is case insensitive, `NMFS_PopID` and `nmfs_popid` are the same, but the column names in the returned tables will all be lower case. The `rcax_hli_xport()` function is used which returns the table with extra meta data, like `NMFS_PopID` included.
```{r}
tab <- rcax_hli_xport("NOSA",
