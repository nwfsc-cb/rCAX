...
#' @param GETargs A list of arguments for the `rcax_GET()` call. These do not normally need to be specified by the user. `table_id` This is the CAX table id. It is looked up using `tablename` and `caxpops`. `recordloc` This is the name of the record we want in the list returned from the GET call. `key` is the API key. See `rcax_GET()` for API key details.
